...
换图不用改文件名。图片是 max-age=604800 的强缓存，靠 URL 上的 ?v= 破缓存 ——
server.js 启动时把这个目录哈希一遍（tutorialVersion），替换掉 help-page.js 里的
__ASSETV__。内容一改哈希就变，老用户下次打开教程就会重新下载。
所以 <img> 的 src 必须写成 /tutorial/xxx.jpg?v=__ASSETV__，漏了那张图就永远不更新。
