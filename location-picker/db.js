...
const DEFAULT = {
  ...
  longitude: -122.00902,
  altitude: 530,
  // 39 是脚本上游的默认值，实测下来在户外必输给 GPS——iPhone 露天能报到 3~5 米，
  // 我们报 39 就等于告诉 iOS「别信我」。改成 3 之后户外才压得住。
  // 3 米在真机范围内（双频 GNSS 本来就能到），不算异常值。
  horizontalAccuracy: 3,
  verticalAccuracy: 1000,
  // 抖动默认关闭。本意是让坐标不要是个纹丝不动的常量（更像真机），
  // 但实测发现它得不偿失：精度抖动是往上加的，把用户好不容易调低的精度
  // 又抬回 GPS 打得过的区间，直接抵消掉唯一能让户外生效的那个杠杆。
  // 抗检测的收益是理论上的，破坏功能是实打实的，所以默认关掉，
  // 想要的人自己在选点页开。
  jitterMeters: 0,
  accJitter: 0
};
