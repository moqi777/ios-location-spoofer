...
<div class="step">
  ...
  <dl class="faq">
    ...
    <dt>原理是什么？会不会被 App 检测到？</dt>
    <dd>iPhone 判断自己在哪，除了 GPS 卫星，还要靠周围的 <b>Wi-Fi 热点和基站</b>。
      后面这条它自己算不出来 —— 手机把扫到的热点列表发给苹果的定位服务器
      （<code>gs-loc.apple.com</code>），由服务器回一个坐标。
      <b>这套方案改的就是这一次回应</b>：小火箭在中间把苹果的回应拦下来，
      把里面的经纬度换成你选的点，再交给系统。</dd>
    <dd>因为改的是<b>系统拿到的原始数据</b>，而不是去骗某个 App，所以：</dd>
    <dd>
      1、<b>是 iOS 自己算出来的结果</b>。所有读定位的 App 拿到的都是同一个值，
        不用一个个去适配，也不存在「这个 App 能改那个 App 不行」。<br>
      2、<b>不越狱、不装插件、不开开发者模式</b>。手机上没有任何被改过的系统文件，
        那些越狱检测、插件检测根本无从触发。<br>
      3、<b>不是「模拟定位」</b>。iOS 15 起，Xcode 和开发者工具模拟出来的位置会被系统
        打上 <code>isSimulatedBySoftware</code> 标记，App 一查就知道 ——
        市面上大部分虚拟定位工具栽的就是这里。我们走的是系统正常的定位流程，
        这个标记不会被置上。</dd>
    <dd style="color:var(--dim);font-size:13px">
      代价就是第 3 步那个 HTTPS 解密必须装好，装好之后就一劳永逸了。</dd>
